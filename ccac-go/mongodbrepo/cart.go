...
func (c cartRepository) Init(userID domain.ID) error {
	res := c.db.Collection(CollectionCart).FindOne(context.TODO(), bson.D{{"user_id", userID}})
	if res.Err() == nil {
		_, err := c.db.Collection(CollectionCart).ReplaceOne(context.TODO(), bson.D{{"user_id", userID}}, &domain.Cart{
...
func (c cartRepository) Get(userID domain.ID) (domain.Cart, error) {
	...
	return cart, c.db.Collection(CollectionCart).FindOne(context.TODO(), bson.M{"user_id": userID}).Decode(&cart)
}
...
func (c cartRepository) DeleteFromCart(userID domain.ID, product domain.Product, cartItem domain.CartItem) error {
	filter := bson.D{{"user_id", userID}, {"items", bson.D{{"$elemMatch", bson.D{{"sku", cartItem.SKU}}}}}}
