...
func (o orderRepository) getByToken(token string) (domain.Order, error) {
	for _, order := range o.orders {
		if order.Cart.Token == token {
			return order, nil
		}
	}
	return domain.Order{}, fmt.Errorf("missing order with token: %s", token)
}
