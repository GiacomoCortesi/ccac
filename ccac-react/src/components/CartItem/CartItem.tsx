import {Avatar, ImageListItem, ListItem, ListItemAvatar, ListItemText, Typography} from "@mui/material";
import {ICartItem} from "../../models/cart";
import {useGetProduct} from "../../services/api-product-service";
import React, {Fragment} from "react";
import Box from "@mui/material/Box";
import {useAddToCart, useDeleteCartItem} from "../../services/api-cart-service";
...
export default function CartItem({product_id, sku, quantity, total}: ICartItem) {
        ...
        {isLoading && <Loading/>}
            {data &&
                ...
                <ListItemAvatar>
                    ...
                    <Avatar variant={"rounded"} src={data.images[0]} alt={data.id} sx={{ width: 92, height: 92 }}/>
                    }
                </ListItemAvatar>
                <ListItemText
                    sx={{margin: "0em 2em"}}
                    ...
                    data.variations.map((variation) => {
                        return variation.sku === sku && variation.options.color + " " + variation.options.size
                    })
                } />
                <IconButton
                ...
                { total && data.price && <Typography variant={"h6"} >{total.value} {data.price.currency}</Typography>}
